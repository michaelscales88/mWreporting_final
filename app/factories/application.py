...
def create_application(*cfg):
    ...
    app_instance.config.from_object('app.config.celery_config.Config')
    app_instance.config.from_object('app.config.default_config.DevelopmentConfig')
    ...
    if not app_instance.debug:
        app_instance.config.from_object('app.config.default_config.ProductionConfig')
